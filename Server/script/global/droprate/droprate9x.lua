...
for i=1,dropnum do
	...
	elseif(nSel == 9) then
		...
		nSel = RANDOM(1000);
		if (nSel==999) then
			DropNpcItem(NpcIndex, 0, 2,7,0,0,5,0,0);--phi phong
		elseif(nSel == 899) then
			DropNpcItem(NpcIndex, 0, 4,15,0,0,5,0,0) --bikip cui`
		elseif(nSel == 799) then
			DropNpcItem(NpcIndex, 0, 4,25,0,0,5,0,0) --bikip cui`
		elseif(nSel == 699) then
			DropNpcItem(NpcIndex, 0, 4,30,0,0,5,0,0) --bikip cui`
		else
		bluedetail = RANDOM(9);
		blueparti = RANDOM(ITEMLIST[bluedetail+1]);
		DropNpcItem(NpcIndex, 0, 0,bluedetail,blueparti,RANDOM(8,10),ser,pluck,
