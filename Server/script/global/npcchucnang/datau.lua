...
[1]={"Lam Thñy Tinh",3,13,0,0},	--Lam thuy tinh
[2]={"Tö Thñy Tinh",3,14,0,0},	--Tu thuy tinh
[3]={"Lôc Thñy Tinh",3,15,0,0},	--Luc thuy tinh
[4]={"Tinh Hång B¶o Th¹ch",3,20,0,0},	--tinh hong bao thach
[5]={"Phóc Duyªn Lé (TiÓu)",2,0,0,0},	--phuc duyen
[6]={"Phóc Duyªn Lé (Trung)",2,1,0,0},	--phuc duyen
[7]={"Phóc Duyªn Lé (§¹i)",2,2,0,0},	--phuc duyen
[8]={"Tèng Kim Chiªu th­ ",2,11,0,0},	--tong kim chieu thu
[9]={"Tiªn Th¶o Lé ",2,45,0,0},	--tien thao lo
[10]={"Thiªn s¬n  B¶o Lé ",2,46,0,0},	--thien son bao lo
[11]={"B¸ch Qu¶ Lé ",2,47,0,0},	--bach qua lo
[12]={"QuÕ Hoa Töu",2,48,0,0},	--que hoa tuu
[13]={"Phi Phong",2,7,0,0},	--phi phong
[14]={"L­îng Ng©n kho¸ng",4,52,0,0},	--luong ngan
[15]={"XÝch §ång kho¸ng",4,53,0,0},	--xich dong
[16]={"Tõ ThiÕt kho¸ng",4,54,0,0},	--tu thiet
[17]={"HuyÒn Tinh kho¸ng th¹ch",3,0,1,0},	--huyen tinh
[18]={"HuyÒn Tinh kho¸ng th¹ch",3,0,2,0}	--huyen tinh
};
...
FIND_ITEM = "T×m cho d· tÈu 1 c¸i %s."
FIND_MAGIC = "T×m cho d· tÈu: %s %s %d-%d."
...
--Chua nhan nhiem vu gi--
	if (bAccept == 0) then
	Say2(format("<color=cyan>D· TÈu<color> : VÞ hiÖp kh¸ch h«m nay ®· hoµn thµnh <color=yellow>%d<color> nhiÖm vô, cã thÓ thùc hiÖn l¹i <color=yellow>%d<color>, ng­¬i muèn lµm n÷a kh«ng?",nDone,MAX_QUEST_PERDAY-nDone),2,1,"",
...
--Da nhan nhiem vu roi--
	...
	if(nKind ==6 or nKind == 7) then
		strPrivate = format("H·y ®Õn %s t×m gióp ta <color=yellow>%d<color> quyÓn %s", GetSubWorldName(GetNumber3(GetTask(TASK_DIADOCHI),1)), GetTask(TASK_DATAU_NEED), strName)
	elseif(nKind == 5) then
		...
		elseif(x == 2 or x == 3) then
			...
			strPrivate = format("H·y t×m cho ta mãn nµy:<color=yellow> %s<color> %s. Nhá nhÊt:%d Lín nhÊt:%d",strDetail, KIND_EQUIP_HAVEMAGIC[nNum][2],GetNumber3(GetTask(TASK_DIADOCHI),1),GetNumber3(GetTask(TASK_DIADOCHI),2));
		end
		strMain = format("§©y lµ nhiÖm vô thø <color=green>%d<color>,®· liªn tiÕp hoµn thµnh <color=green>%d<color> nhiÖm vô. %s",nDone+1,GetTask(TASK_DATAU_NUM),strPrivate);
	end
		Say2(strMain,6,1,"",
...
function xacnhanhuy1()
	Say2("Ng­¬i  suy nghÜ kü ch­a, muèn bá thËt sao?",2,1,"",
...
function nhannv()
	...
	if( kindrand == 0) then --loai ko giao
		...
		SetTask(TASK_DATAU_ACCEPT,SetNumber(nTaskValue, 3,nNeedNum-1))--set « [3] ®é khã
		...
		elseif(i == 2) then --phóc duyªn
			nNeedNum = LEVEL_UPDATE[nLevel][j][1+nNeedNum]
			SetTask(TASK_DATAU_NEED,nNeedNum);--so luong can update
			SetTask(TASK_DATAU_OLD,GetBless());--so luong cò
		...
		local str = format("D· tÈu: n©ng cÊp %d %s.", nNeedNum, QUESTKIND_UPDATE[i][2])
		AddNote(str);
		main();
	return end
...
function check()
	...
	PutItem("VÞ hiÖp kh¸ch ®· cã vËt phÈm ta cÇn råi chø?","y/giao","n/no")
end;
...
function thuong()
	...
	GetReward("Ng­¬i lµm tèt l¾m! H·y nhËn lÊy phÇn th­êng cña ng­¬i!",
...
function ClearTask()
	...
	if(nDone == 29) then
		if(CheckRoom(2,4) == 1 or CheckRoom(4,2) == 1 or CheckRoom(3,3) == 1) then
			...
			Msg2Player("<color=green>B¹n nhËn ®­îc phÇn th­ëng D· TÈu.")
		end
	end
end
