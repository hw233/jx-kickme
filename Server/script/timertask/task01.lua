...
function ontime_vuotai(nIndex, nGlbMission,nMap)
	StopMissionTimer(nIndex,1);--tat hen gio so 1
	SetGlbMissionV(nGlbMission-1, 0)--set lai chua dang ky cho lan sau
	if(GetMSGroupCount(nIndex,0) == 0) then--map nay ko ai tham gia->close
		CloseMission(nIndex);
	return end
...
function ontime_liendau(nIndex)	--map 396
	StopMissionTimer(nIndex,1);--tat hen gio so 1
	SetMissionV(0, 0)--set lai chua dang ky cho lan sau(so nguoi dang ky)
	local nCount = GetMSPlayerCount(nIndex);
	if(nCount == 0) then
	 CloseMission(nIndex)
	return end
	...
	repeat
		...
		if( nP1 == 1 and nP2 == 1) then
			...
			SetCurCamp(taphai) --set phe phai'
			SetPunish(1)
			AddMSPlayer(1,2)
		elseif(nP1 == 1) then
